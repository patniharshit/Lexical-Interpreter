  ...
  (lambda (ast env)
    (cases Ast ast 
      ...
      [assume& (binds body)   (let ((tpls (rev-reduce  (lambda(x u) 
                                                   (extended-env (mk-tuple  (first u)  
                                                              (expressible->denotable 
                                                                  (eval-ast (second u) env))) x)) 
                                          env binds))) ;evaluate asts to get values to be bound to identifiers

                                  (eval-ast body tpls))] ;evaluate the body in the extended envirnment
